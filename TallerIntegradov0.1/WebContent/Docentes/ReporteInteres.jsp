...
$('#divCurso').on('change', '#selectCurso', function(){
	//$('#selectSemestre').val($('#selectSemestre > option:first').val());
	listarDocentexInteres();
});
...
$('#btnEXCEL').click(function(){ 
	...
	$.ajax({
		type : 'POST',
		url : '/TallerIntegradov0.1/ReporteDocentexInteres',
		data : {
...
function listarDocentexInteres(){
	$('#myTable tbody tr').remove();
	var idcurso=$('#selectCurso').val(); 
	var sem =$('#selectSemestre').val();
	$.ajax({
        type: 'POST',
        url: '/TallerIntegradov0.1/ListarDocentexInteres',
        data : {
			idcurso : idcurso,
			sem : sem 
		},
